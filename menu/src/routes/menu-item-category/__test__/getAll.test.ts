...
it('should return 200 and no Documents when user does not own the Document', async () => {
  ...
  const response = await request(app)
    .get(`${API_ROOT_ENDPOINT}/menu/categories/`)
    .set('Cookie', cookieTwo)
    .send({})
    .expect(200);

  expect(response.body.data.menu_item_categories.length).toEqual(0);
});
...
it('should return 200 and all Documents when user is an Admin', async () => {
  ...
  expect(response.body.data.menu_item_categories.length).toEqual(1);
});
